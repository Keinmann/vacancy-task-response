...
function App() {
  ...
  const decode = input => {
    var array = input.match(/([.|-]\d*)/g)
      .map(x => {
        let str = x.match(/\.|-/g)[0] == '.' ? '1' : '0';
        let rep = x.match(/(\d*)/g).join('');
    ...
    for (let i = 0; i < array.length; i++) {
      if (array[i] < 0) {
        newArray.push(num.join(''));
        num = [];
        continue;
      }
      else num.push(array[i])
      if (i == array.length - 1) newArray.push(num.join(''))
    }
    array = newArray;
    num = [];
    newArray = [];
